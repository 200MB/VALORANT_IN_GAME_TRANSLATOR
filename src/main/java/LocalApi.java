...
public class LocalApi {
    ...
    public void getPartyChatInfo() {
        ...
        String encodeBytes = Base64.getEncoder().encodeToString("riot:%s".formatted(lockFileIO.getPassword()).getBytes());
    ...
    public void sendChat(String cid, String message) throws UnsupportedEncodingException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(request)) {
            ...
            String result = EntityUtils.toString(entity);
            System.out.println(result);
            System.out.println(GREEN_ANSI + "SENT MESSAGE" + RESET_ANSI);
        } catch (IOException e) {
            e.printStackTrace();
        }
